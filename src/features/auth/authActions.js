  ...
  async (data, { rejectWithValue }) => {
    try {
      ...
      const response = await ApiClient.post('/auth/guest/session', {
        ...
        target_weight_kg: data.weight,
        target_calories: data.customCalories || data.calorieGoal,
        target_protein_g: data.macros?.protein || 0,
        target_carbs_g: data.macros?.carbs || 0,
        target_fats_g: data.macros?.fat || 0,
